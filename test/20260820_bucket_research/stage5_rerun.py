...
signal_df['_abFlag'] = signal_df['Date'].isin(ab_dates) * 1
...
keep = ['Date','QuoteCode','TransTime','ChannelSeq','_abFlag'] + [c for c in signal_df.columns if c.endswith('_pred') or c.endswith('_ABpred')]
signal_df[keep].to_parquet(f'{OUT}/stage5_pooled_preds.parquet', index=False)
print('preds saved:', [c for c in keep if 'pred' in c], flush=True)
print('STAGE5 BUILD+RUNS COMPLETE', flush=True)
