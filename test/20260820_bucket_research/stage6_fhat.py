...
files = [f for f in files if '20250903.parquet' <= f <= '20260817.parquet']
print(f'files: {files[0]} .. {files[-1]}  n={len(files)}', flush=True)

schema_cols = pl.scan_parquet(f'{DATA}/{files[-1]}').collect_schema().names()
pre_existing_re = [c for c in schema_cols if ('_re' in c) and ('2330' not in c) and c != 'txf_to_1330_return']
...
# ---------------- cell 7: labels ----------------
# ---- conditional baseline f-hat(RemainSeconds, ToRef): prior-20-day cell means, shift(1) so
# ---- today's rows never see today's data. Coordinates are known at event time. No leak.
T_EDGES = [3300, 4800, 6300, 7800, 9300, 10800, 12300, 13800, 99999]
R_EDGES = [-0.015, -0.005, 0.0, 0.005, 0.01, 0.015, 0.02, 0.03, 0.05]
SHRINK_K = 2000.0
signal_df['_tbin'] = pd.cut(signal_df.RemainSeconds, T_EDGES, labels=False)
signal_df['_rbin'] = pd.cut(signal_df.ToRef, R_EDGES, labels=False)
signal_df['_cell'] = signal_df['_tbin'] * 100 + signal_df['_rbin']
_z = signal_df['residual_TakerSell_CloseBP']
_ok = np.isfinite(_z)
_day = (signal_df[_ok].groupby(['Date','_cell'])['residual_TakerSell_CloseBP']
        .agg(['sum','count']).unstack('_cell').fillna(0.0).sort_index())
_S = _day['sum'].rolling(20, min_periods=20).sum().shift(1)
_N = _day['count'].rolling(20, min_periods=20).sum().shift(1)
_g = _S.sum(axis=1) / _N.sum(axis=1)
_fh = (_S.add(_g * SHRINK_K, axis=0)) / (_N + SHRINK_K)
_fh_long = _fh.stack().rename('_fhat').reset_index()
signal_df = signal_df.merge(_fh_long, on=['Date','_cell'], how='left')
signal_df['residual_ex'] = signal_df['residual_TakerSell_CloseBP'] - signal_df['_fhat']
print('fhat coverage:', signal_df['_fhat'].notna().mean().round(4), flush=True)

g_sq = signal_df.groupby(['Date','QuoteCode'])
g_d = signal_df.groupby(['Date'])
signal_df['TakerSell_CloseBP_net'] = signal_df['residual_ex'] - g_sq['residual_ex'].transform('mean')
signal_df['TakerSell_CloseBP_netM'] = signal_df['residual_ex'] - g_d['residual_ex'].transform('mean')
...
keep = ['Date','QuoteCode','TransTime','ChannelSeq','_fhat','residual_ex','_tbin','_rbin','TakerSell_CloseBP_net','TakerSell_CloseBP_netM'] + [c for c in signal_df.columns if c.endswith('_pred') or c.endswith('_ABpred')]
signal_df[keep].to_parquet(f'{OUT}/stage6_fhat_preds.parquet', index=False)
print('preds saved:', [c for c in keep if 'pred' in c], flush=True)
print('STAGE6 BUILD+RUNS COMPLETE', flush=True)
