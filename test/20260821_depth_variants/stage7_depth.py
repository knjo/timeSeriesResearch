...
for f in files:
    ...
    lf = lf.with_columns(
        ...
        & (pl.col('FillLots_atLow') / (pl.col('avg_bidLots1') + pl.col('avg_askLots1')) < -2)
...
signal_df[keep].to_parquet(f'{OUT}/stage7_depth_preds.parquet', index=False)
print('preds saved:', [c for c in keep if 'pred' in c], flush=True)
print('STAGE7 BUILD+RUNS COMPLETE', flush=True)
