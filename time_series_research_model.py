...
for f in tqdm (file_names[-300:]): 
    ...
    rankThres_btn = 0
    rankThres = 150
    stock_df = pd.read_parquet(folder_path / f)[lambda x : x[targetLabel] * x.BidPrice1 > 0]
    try : 
...
with (MODULE_DIR / "abnormal_dates.json").open("r") as f:
...
day_columns = []
from src.research.timeSeries.model_analysis import plot_model_dashboard
